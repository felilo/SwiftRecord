...
public extension String
{
    ...
    public func replaceRegex(pattern:String, with replacement:String) -> String {
        ...
        return map(regexResult: self =~ Regex(pattern), replacementTemplate: replacement)
    }
}
...
extension String
{
    ...
    func substringWithRange (range:NSRange) -> String {
        //return substringWithRange(range: convertRange(range))
        return substringWithRange(range: convertRange(nsrange: range))
    }
    ...
    func convertRange (range: Range<Int>) -> Range<String.Index> {
        ...
        let end = self.index(start, offsetBy: range.endIndex - range.startIndex)
...
public struct Regex
{
    ...
    public func replaceMatchesIn (string:String, with replacement:String) -> String {
        return map(regexResult: (string =~ self), replacementTemplate: replacement)
    }
}
    ...
    public init (regex r:NSRegularExpression, searchString s:String, items i:[NSTextCheckingResult])
    {
        ...
        captures = items.flatMap { result in
            (0 ..< result.numberOfRanges).map { i in
                ...
                return s.substringWithRange(range: nsrange)
            }
        }
    }
